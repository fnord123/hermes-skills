...
imports, a script that does not parse, an unreadable SKILL.md whose rules
cannot be checked), broken routing (no PREFER clause, no trigger list, or a
...
sentence verbatim), or a broken script contract (a documented entry point
whose output carries no `"ok"` field, whose `main` has no top-level exception
guard, or which can never exit non-zero on failure — all three block, because
the model then cannot tell success from failure by a stable rule).
