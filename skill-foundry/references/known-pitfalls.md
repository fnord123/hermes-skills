...
- **A card blocked at park is never re-armed — so it is never
  reconciled on its own.** The substrate returns an idempotency key's
  existing card regardless of its status, and nothing ever unblocks a
  parked card. Two observed instances: (1) a resume re-dispatched the
  same (skill, issue, role, round) and surfaced the EXISTING BLOCKED
  card as the live next card, so the pipeline sat dead until the owner
  unblocked it by hand; (2) an operator merge finished a parked-commit
  pipeline and its cleanup deleted the worktree, but the commit card
  the role had blocked stayed blocked forever, pointing at a deleted
  worktree. Rule that closes it: a dispatch that hits an existing
  `blocked` card supersedes it — comment the card `SUPERSEDED` and
  create the live card under a fresh key (never unblock: the owner's
  hand is the only unblock path) — and the operator merge completes
  the run's remaining blocked cards with an owner-completion note
  (what happened, via what). Completion is reconciliation, not
  unblocking: it records the outcome on the closing run and re-arms
  nothing.
