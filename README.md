...
| [`daily-briefing`](./daily-briefing/) | Cron-driven morning briefing pipeline (calendar / weather / news / markets / lottery) that posts to Discord via webhook. Companion skill lets the agent edit the pipeline's policy files (trusted sources, news topics, watchlist tickers, calendar people) in plain English. The pipeline itself lives in the companion repo [`fnord123/daily-briefing`](https://github.com/fnord123/daily-briefing) at `~/daily-briefing/` — source of truth for the code and the policy files. |
