...
# One token per process: each script call is one stage, and the helper's
# on-disk cache (55 min) already makes re-mints cheap across stages.
_ROLE_TOKEN = None
...
def _role_token() -> str:
    ...
    run(check=True); it must never silently fall back to the operator's
    PAT, because a silent fallback is a misattribution, not a retry.
    """
    global _ROLE_TOKEN
    if _ROLE_TOKEN is None:
        ...
        _ROLE_TOKEN = proc.stdout.strip()
        if not _ROLE_TOKEN:
            fail("skillpipe-auth returned an empty token")
    return _ROLE_TOKEN
